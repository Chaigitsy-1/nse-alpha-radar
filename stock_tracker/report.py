...
def _actionable_scores(scores: list[CompanyScore]) -> list[CompanyScore]:
    ...
    for score in _filtered_top_overall(scores):
        ...
        has_core = bool(categories & {"capex_lifecycle", "turnaround", "financial_quality", "filing_validation", "ai_validation"})
        has_confirmation = bool(categories & {"trend_momentum", "technical_volume", "management_guidance", "filing_validation", "ai_validation"})
...
def _action_for_score(score: CompanyScore) -> tuple[str, str]:
    ...
    has_quality = bool(categories & {"financial_quality", "long_term_tailwind", "management_guidance", "capex_lifecycle", "filing_validation", "ai_validation"})
...
def _quality_tier_for_score(score: CompanyScore) -> tuple[str, str]:
    ...
    has_validation = bool(categories & {"filing_validation", "ai_validation"})
    ...
    has_quality_context = bool(categories & {"capex_lifecycle", "turnaround", "financial_quality", "management_guidance", "long_term_tailwind", "filing_validation", "ai_validation"})
...
def _thesis_for_score(score: CompanyScore) -> str:
    ...
    for signal in top:
        ...
        elif signal.category == "ai_validation":
            parts.append("AI validation found material evidence in the filing; still review source evidence before action.")
...
def _next_steps(score: CompanyScore) -> list[str]:
    ...
    if "ai_validation" in categories:
        steps.append("  - Review the AI evidence and manual checks against the original filing before sizing any position.")
